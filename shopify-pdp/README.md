...
Every section has `background_color` and `text_color` color pickers in the theme editor — no code editing needed to change the look. The default palette is a warm, playful take rather than flat black-and-white: seven sections default to a banana-cream background (`#fff8ec`) with deep grape-plum text (`#241134`); `ingredients-showcase`, `faq-accordion`, `newsletter-footer`, and `announcement-bar` default to that same plum as the background with cream text, giving the page an alternating light/dark rhythm out of the box. Any section can be flipped either way, or recolored entirely, from the theme editor — these are just starting values.

How it's wired so a color change doesn't quietly break contrast elsewhere:
- "Inverted" elements that sit on top of the section (solid buttons, badges, the popular-tier ribbon) use `text_color` as their own background and `background_color` as their own text, so a solid plum button on a cream section automatically becomes a solid cream button if you flip the section to a plum background — it always contrasts against whatever you pick, instead of assuming light-on-dark.
...
- Data/status colors are fixed, vibrant accents kept separate from `background_color`/`text_color` since they're semantic, not brand palette: raspberry-pink `#ff4d6d` (sale), emerald-mint `#14b876` (save), amber-gold `#ffb000` (star rating), and vivid orange `#ff8a3d` (selected swatch / hover accent) — swap these in `main-product.liquid`'s `--pdp-red`/`--pdp-green`/`--pdp-gold`/`--pdp-accent` custom properties (and the matching literal colors in the few other sections that reference them) if you want a different accent set.
